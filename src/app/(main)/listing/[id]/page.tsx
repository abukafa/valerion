...
export default async function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const image = listing.images?.[0] || "https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800&q=80";

  return (
    <div className="max-w-4xl mx-auto px-4 py-6 pb-24 md:pb-6">
      <Link href="/" className="inline-flex items-center text-primary hover:underline mb-6 text-sm font-medium">
        ...
        actionButtons={
          <>
            ...
            {listing.status === "SOLD" || listing.status === "SUSPENDED" ? (
              <button disabled className="flex-[2] bg-red-500/20 text-red-500 font-bold py-3 md:py-4 rounded-xl flex items-center justify-center gap-2 border border-red-500/30 cursor-not-allowed">
                TERJUAL
              </button>
            ) : listing.isBooked ? (
              <button disabled className="flex-[2] bg-orange-500/20 text-orange-500 font-bold py-3 md:py-4 rounded-xl flex items-center justify-center gap-2 border border-orange-500/30 cursor-not-allowed">
